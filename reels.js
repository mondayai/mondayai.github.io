(function () {
    ...
    const DEBUG = false;
    const log = (...a) => { if (DEBUG) console.log('[YT_REELS]', ...a); };
    ...
    let scrollLocked = false, savedScrollY = 0, touchBlocker = null;
    ...
    let modalEl = null, modalPlayer = null, modalGestures = null;
    ...
    function initAll() {
        ...
        if (!listenersBound) {
            ...
            document.addEventListener('keydown', (e) => {
                ...
                else if (e.key === 'ArrowRight' || e.key === 'ArrowUp') { e.preventDefault(); playModalNext(); }
                else if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') { e.preventDefault(); playModalPrev(); }
                else if (e.key === ' ') { e.preventDefault(); toggleModalPlayback(true); }
            });
        }
    ...
    function ensurePlayerAndPlay(iframe) {
        ...
        const attach = () => {
            if (!window.YT || !window.YT.Player) { setTimeout(attach, 20); return; }
            const id = iframe.id;
            const wrapper = iframe.closest('.yt-embed');
            if (!players.has(id)) {
                try {
                    const p = new YT.Player(id, {
                        videoId: extractYtId(iframe.src) || extractYtId(iframe.dataset.ytid),
                        playerVars: {
                            autoplay: 1,
                            mute: 1,
                            controls: 0,
    ...
    function playNextInFeed(currentId) {
        ...
        try {
            next.wrap.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
        } catch (_) { }
        setTimeout(() => { ensurePlayerAndPlay(next.iframe); }, SCROLL_TO_NEXT_MS);
    }
    ...
    function ensureModal() {
      ...
      <div class="yt-modal__inner">
        ...
        modalEl.querySelector('.yt-modal__close').addEventListener('click', closeModal);

        prevBtn.addEventListener('click', (e) => { e.stopPropagation(); lastGestureTs = Date.now(); playModalPrev(); });
        nextBtn.addEventListener('click', (e) => { e.stopPropagation(); lastGestureTs = Date.now(); playModalNext(); });

        attachSwipe(modalGestures, {
            onTap: () => { lastGestureTs = Date.now(); toggleModalPlayback(true); },
            onSwipeUp: () => { lastGestureTs = Date.now(); playModalNext(); },
            onSwipeDown: () => { lastGestureTs = Date.now(); playModalPrev(); },
            blockVertical: true
        });

        modalGestures.addEventListener('click', () => {
    ...
    function openModalAtIndex(idx, startSeconds) {
        ...
        const startModal = () => {
            if (!window.YT || !window.YT.Player) { setTimeout(startModal, 20); return; }
            if (modalPlayer) {
                try {
                    modalPlayer.loadVideoById({ videoId: vid, startSeconds: startSeconds || 0 });
                    modalPlayer.mute();
                ...
                modalPlayer = new YT.Player('yt-reels-modal-player', {
                    ...
                    events: {
                        ...
                        onStateChange: (e) => {
                            if (AUTO_ADVANCE_MODAL && e.data === YT.PlayerState.ENDED) playModalNext();
                        }
                    }
                });
            }
    ...
    function playModalNext() {
        ...
        if (nextIdx >= playlist.length) {
            if (!WRAP_AT_END) { closeModal(); return; }
            nextIdx = 0;
        }
        modalIndex = nextIdx;
    ...
    function closeModal() {
        ...
        try { modalPlayer && modalPlayer.stopVideo && modalPlayer.stopVideo(); } catch (_) { }
        unlockScroll();
    }
    ...
    function resizeModalPlayer() {
        try { modalPlayer && modalPlayer.setSize(window.innerWidth, window.innerHeight); } catch (_) { }
    }
    ...
    function getVideoId(iframe) {
        ...
        if (id) return id;
        const ds = iframe.getAttribute('data-src');
        if (ds) { id = extractYtId(ds); if (id) return id; }
        if (iframe.src) { id = extractYtId(iframe.src); if (id) return id; }
        return null;
    }
    ...
    function safePlay(id) {
        ...
        if (!iframe) return;
        if (!p) { ensurePlayerAndPlay(iframe); return; }
        try {
            p.mute();
            p.playVideo();
    ...
    function safePause(id) {
        ...
        const iframe = document.getElementById(id);
        const wrap = iframe && iframe.closest('.yt-embed');
        if (!p) { desired.set(id, 'pause'); return; }
        try {
            p.pauseVideo();
    ...
    function attachSwipe(el, opts) {
        ...
        const onEnd = (e) => {
            if (!tracking) return;
            tracking = false;
            const p = (e.changedTouches && e.changedTouches[0]) || (e.touches && e.touches[0]) || e;
            ...
            if (Math.abs(dy) > Math.abs(dx) * VERTICAL_BIAS) {
                ...
                if (Math.abs(dx) >= SWIPE_MIN_DIST && vel >= SWIPE_MIN_VEL) {
                    if (dx < 0 && opts.onSwipeLeft) opts.onSwipeLeft && opts.onSwipeLeft();
                    else if (dx > 0 && opts.onSwipeRight) opts.onSwipeRight && opts.onSwipeRight();
                    return;
                }
            }
